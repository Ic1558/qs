...
class AuthoringFlowTests(unittest.TestCase):
    ...
    def test_api_importable_and_export_fails_cleanly_without_qs_engine(self) -> None:
        import universal_qs_engine.api as api_module
        from universal_qs_engine.qs_engine_adapter import _load_qs_engine

        self.assertIsNotNone(api_module)
        _load_qs_engine.cache_clear()

        blocked = {k: None for k in list(sys.modules) if k.startswith("qs_engine")}
        blocked["qs_engine"] = None

        tools_root = str(Path(__file__).resolve().parents[3] / "tools")
        patched_path = [p for p in sys.path if p != tools_root]

        try:
            with patch.dict(sys.modules, blocked), patch.object(sys, "path", patched_path):
                with self.assertRaises(ModuleNotFoundError) as ctx:
                    _load_qs_engine()
                self.assertIn("qs_engine is not installed", str(ctx.exception))
        finally:
            _load_qs_engine.cache_clear()
